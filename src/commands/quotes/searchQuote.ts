...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) => option.setName('from').setDescription('From').setRequired(false))
		.addStringOption((option) => option.setName('to').setDescription('To').setRequired(false)),
	...
	async execute(interaction) {
        ...
        `
			...
			const matches = quotesArray.filter((quote) => {
				const similarResult = stringSimilarity.compareTwoStrings(quote.text, search);
			...
			const matchesText = matchesArray
				.map((match) => `"${match.text}" - <@${match.userid}> as #${match.rownum}`)
				.join('\n');
			...
			interaction.reply({
				content: 'Search failed, try reducing the range if you are setting it high.',
				flags: MessageFlags.Ephemeral
			});
		}
	}
